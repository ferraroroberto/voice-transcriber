...
Modeled after `grocery-shopping-automation/gen_ssl_cert.py`. Output goes
to `webapp/certificates/`:
...
MOBILECONFIG_FILENAME = "voice-transcriber-ca.mobileconfig"
...
def main() -> int:
    ...
    # Mirror into the static dir so the webapp can serve it at /install-ca.
    ...
    logger.info(f"   • Restart webapp.bat / tray.bat — uvicorn will pick up the new cert.")
    logger.info(f"   • iOS: open  https://<host>:8443/install-ca")
    logger.info(f"     then Settings → General → VPN & Device Management → install profile,")
    logger.info(f"     then Settings → General → About → Certificate Trust Settings → enable.")
...
def _local_hostnames() -> Set[str]:
    ...
    try:
        ...
        pass
    # Tailscale: best-effort `tailscale status --self --json`
    try:
...
def _local_ip_addresses() -> Set[ipaddress.IPv4Address]:
    ...
    # Probe by external connect — gets the routed-out IP on the LAN.
    try:
...
def _build_leaf(ca_key, ca_cert, hostnames: Set[str], ips: Set[ipaddress.IPv4Address]):
    key = rsa.generate_private_key(public_exponent=65537, key_size=2048)
    subject = x509.Name([
        x509.NameAttribute(NameOID.COMMON_NAME, "voice-transcriber.local"),
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, CA_ORG),
    ])
...
def _build_mobileconfig(ca_cert) -> bytes:
    """Build an iOS .mobileconfig profile that installs the CA cert.

    Unsigned profile — iOS will warn "Unverified" once during install,
    same as any self-signed CA workflow. After install, the user goes
    to Settings → General → About → Certificate Trust Settings to
    enable full trust on the CA.
    """
...
<dict>
    ...
    <array>
        <dict>
            ...
            <string>voice-transcriber-ca.cer</string>
            <key>PayloadContent</key>
            <data>
{cert_b64_chunks}
            </data>
            <key>PayloadDescription</key>
            <string>Adds the Voice Transcriber local CA to the iOS trust store.</string>
            <key>PayloadDisplayName</key>
            <string>{CA_COMMON_NAME}</string>
            <key>PayloadIdentifier</key>
            <string>com.voicetranscriber.localca.cert.{payload_uuid}</string>
            <key>PayloadType</key>
    ...
    <key>PayloadDescription</key>
    <string>Trust profile for self-signed Voice Transcriber webapp on this LAN.</string>
    <key>PayloadDisplayName</key>
    <string>Voice Transcriber Trust</string>
    <key>PayloadIdentifier</key>
    <string>com.voicetranscriber.localca.profile.{profile_uuid}</string>
...
def _install_windows_trust(ca_pem: Path) -> None:
    """Install ca.pem into Windows CurrentUser\\Root via certutil (no admin needed)."""
    try:
        result = subprocess.run(
